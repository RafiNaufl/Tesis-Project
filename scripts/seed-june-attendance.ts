...
async function main() {
    console.log('=== Seeding June 2026 attendance for Rahmat R & Johan Irawan ===');

    const startDate = new Date(2026, 5, 1); // June 1, 2026
    const endDate = new Date(2026, 5, 30); // June 30, 2026

    // Get both employees using known IDs
    ...
    // Clean existing attendance for June 2026 for both employees
    console.log('Cleaning existing June 2026 data...');
        ...
        while (currentDate <= endDate) {
            ...
            if (isWeekend) {
                ...
                if (absentRoll < 0.07) { // 7% absent
                    ...
                    const isLate = lateRoll < 0.15; // 15% late

                    // Generate check-in time
                    let checkInTime;
                    if (isLate) {
                        // Late: 08:30 - 09:15
                        checkInTime = randomTime(8, 45);
                    ...
                    const hasOvertime = overtimeRoll < 0.25; // 25% overtime
